...
function getAdminCustomerAvatarSrc($profile_image, $customer_name, $gender = '', $race = '') {
    $default_avatar_url = "https://ui-avatars.com/api/?name=" . urlencode($customer_name) . "&background=20304a&color=fff&size=80&rounded=true&bold=true";
    ...
    if (!empty($profile_image)) {
        ...
        elseif (strpos($profile_image, 'images/') === 0) {
            $src = "../" . $profile_image;
        } 
        elseif (strpos($profile_image, '/') === 0) {
            ...
            $src = "../uploads/customer_avatars/" . $profile_image;
        }
    ...
    $default_filename = "defaultAvatar_{$prefix}{$gender}.png";
    $default_path = "../uploads/customer_avatars/" . $default_filename;
    if (file_exists($default_path)) {
        return $default_path . '?v=' . filemtime($default_path);
    }
    ...
    if ($range == 'weekly') {
        for ($i = 3; $i >= 0; $i--) {
            ...
            $sql = "
                ...
                FROM invoices
                WHERE status = 'Paid'
                AND DATE(paid_date) BETWEEN '$start' AND '$end'
            ";
...
<div class="main-content">
    ...
    <?php if ($view == 'dashboard'): ?>
        <?php
            ...
            $qtn_query = "SELECT qtn_id FROM quotations WHERE status = 'Accepted'";
            $qtn_res = mysqli_query($conn, $qtn_query);
            if ($qtn_res && mysqli_num_rows($qtn_res) > 0) {
                while ($qtn_row = mysqli_fetch_assoc($qtn_res)) {
                    $qtn_id = $qtn_row['qtn_id'];
                    $check = $conn->query("SELECT id FROM project_progress 
                                        WHERE qtn_id = $qtn_id 
                    ...
                    <div class="modal-body p-4">
                        <form action="" method="POST" enctype="multipart/form-data" id="regStaffForm" onsubmit="return validateStaffPasswords()">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="email" name="staff_email" id="staff_email" class="form-control" required>
                                <div class="password-helper" id="staff_email_helper">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="tel" name="staff_phone" id="staff_phone" class="form-control" required>
                                <div class="password-helper" id="staff_phone_helper">
                ...
                <table class="table quote-invoice-table align-middle" id="qiTable" style="font-size: 0.85rem;">
                    ...
                    <tbody>
                        <?php foreach ($customers as $cust):
                            $avatar_src = getAdminCustomerAvatarSrc($cust['profile_image'] ?? '', $cust['name'], $cust['gender'] ?? '', $cust['race'] ?? '');
...
<?php
    ...
    $cust_res = $conn->query("
        SELECT customer_id, name, email, phone, address,
               gender, race, status, profile_image
        FROM customers
        WHERE status = 1
        ORDER BY name ASC
    ");
    ...
    while ($cust = $cust_res->fetch_assoc()) {
        ...
        $qtns = $conn->query("
            SELECT qtn_id, qtn_number, staff_id, created_at
            FROM quotations 
            WHERE customer_id = $cust_id AND status = 'Accepted'
            ORDER BY created_at DESC
        ");

        $total_qtns = $qtns->num_rows;
        $completed_qtns = 0;
        $quotation_progress = [];
        ...
        if ($total_qtns > 0) {
            $first = true;
            while ($qtn = $qtns->fetch_assoc()) {
                $qtn_id = $qtn['qtn_id'];
                if ($first) {
                    $primary_staff_id = $qtn['staff_id'];
                    $first = false;
                }
                $check = $conn->query("
                    ...
                    AND status = 'Completed'
                ");
                $is_completed = ($check && $check->num_rows > 0);
                if ($is_completed) $completed_qtns++;

                $steps_res = $conn->query("
                    SELECT progress_step, status FROM project_progress 
                    WHERE qtn_id = $qtn_id
                ");
                $step_status = array_fill(0, $total_steps, 'Pending');
                while ($step = $steps_res->fetch_assoc()) {
                    $name = $step['progress_step'];
                    if (isset($step_order[$name])) {
                        $idx = $step_order[$name];
        ...
        $entry = [
            ...
            'total_quotations'    => $total_qtns,
            'completed_quotations'=> $completed_qtns,
            'quotation_progress'  => $quotation_progress,
            'primary_staff_id'    => $primary_staff_id, 
        ];

        if ($total_qtns == 0) {
            $ongoing_entries[] = $entry;
        } elseif ($completed_qtns == $total_qtns) {
    ...
    usort($ongoing_entries, function($a, $b) {
        $ratio_a = ($a['total_quotations'] == 0) ? -1 : ($a['completed_quotations'] / $a['total_quotations']);
        $ratio_b = ($b['total_quotations'] == 0) ? -1 : ($b['completed_quotations'] / $b['total_quotations']);
    ...
    <div class="col-12">
        <div class="card-custom h-100">
            <div class="card-header-custom bg-white border-bottom">
                ...
                <span class="badge bg-secondary"><?= count($ongoing_entries) ?> entries</span>
            </div>
            <div class="p-3" style="max-height: 60vh; overflow-y: auto;" id="ongoingCustomersList">
                <?php if (empty($ongoing_entries)): ?>
                    <div class="text-center text-muted py-5">No ongoing projects.</div>
                <?php else: foreach ($ongoing_entries as $entry):
                    $avatar_src = getAdminCustomerAvatarSrc($entry['profile_image'] ?? '', $entry['name'], $entry['gender'] ?? '', $entry['race'] ?? '');
                    $has_quotations = ($entry['total_quotations'] > 0);
                    $completed_ratio = $has_quotations ? round(($entry['completed_quotations'] / $entry['total_quotations']) * 100) : 0;
                    $link_url = ($entry['primary_staff_id']) ? "admin_dashboard.php?view=staff_progress&staff_id=" . $entry['primary_staff_id'] : '#';
                    $onclick = ($entry['primary_staff_id']) ? "window.location.href='$link_url'" : "alert('This customer has no active projects assigned to a staff.')";
                ?>
                    <div class="card mb-3 border shadow-sm customer-card"
                        onclick="<?= $onclick ?>"
                        style="cursor: <?= $entry['primary_staff_id'] ? 'pointer' : 'default' ?>;"
                        data-customer-name="<?= htmlspecialchars($entry['name']) ?>"
                        data-customer-email="<?= htmlspecialchars($entry['email']) ?>"
                        data-customer-phone="<?= htmlspecialchars($entry['phone']) ?>"
                        data-customer-address="<?= htmlspecialchars($entry['address']) ?>">
                        <div class="card-body">
                            <div class="d-flex align-items-center gap-3">
                                <img src="<?= $avatar_src ?>" width="50" height="50" class="rounded-circle"
                                    onerror="this.src='https://ui-avatars.com/api/?name=<?= urlencode($entry['name']) ?>&background=20304a&color=fff&size=60&rounded=true&bold=true';">
                                <div class="flex-grow-1">
                                    <h6 class="mb-0 fw-bold"><?= highlightText2($entry['name'], $search) ?></h6>
                                    <div class="small text-muted">ID: #<?= $entry['customer_id'] ?></div>
                                    <div class="small mt-1">
                                        <i class="bi bi-envelope me-1"></i> <?= highlightText2($entry['email'] ?: 'N/A', $search) ?>
                                        &nbsp;|&nbsp; <i class="bi bi-telephone me-1"></i> <?= highlightText2($entry['phone'] ?: 'N/A', $search) ?>
                                    </div>
                                    <div class="small">
                                        <i class="bi bi-geo-alt me-1"></i> <?= highlightText2($entry['address'] ?: 'No address', $search) ?>
                                    </div>
                                    <div class="small mt-1">
                                        <i class="bi bi-person me-1"></i> <?= htmlspecialchars($entry['gender'] ?: '-') ?> / <?= htmlspecialchars($entry['race'] ?: '-') ?>
                                    </div>
                                    <?php if ($has_quotations): ?>
                                        <div class="mt-2">
                                            ...
                                            <small class="text-muted"><?= $entry['completed_quotations'] ?>/<?= $entry['total_quotations'] ?> quotations completed</small>
                                        </div>
                                        <div class="mt-2 small">
                                            <?php foreach ($entry['quotation_progress'] as $qp): ?>
                                                <div>
                                                    <code><?= $qp['qtn_number'] ?></code>
                                                    <?php if ($qp['is_completed']): ?>
                                                        <span class="badge bg-success">Completed</span>
                                                    <?php else: ?>
                                                        <span class="badge bg-warning text-dark"><?= $qp['completed_steps'] ?>/<?= $qp['total_steps'] ?> steps</span>
                                                    <?php endif; ?>
                                                </div>
                                            <?php endforeach; ?>
                                        </div>
                                    <?php else: ?>
                                        <div class="mt-2">
                                            <span class="badge bg-secondary">No accepted quotation</span>
                                        </div>
                                    <?php endif; ?>
                                </div>
                                <div onclick="event.stopPropagation();">
                                    <a href="?view=customers&toggle_customer=<?= $entry['customer_id'] ?>&status=<?= $entry['status'] == 1 ? 0 : 1 ?>"
                                    class="status-badge <?= $entry['status'] == 1 ? 'status-active' : 'status-inactive' ?>">
                                        <?= $entry['status'] == 1 ? 'Active' : 'Inactive' ?>
                                    </a>
                                    <button class="btn-action d-block mt-2"
                                            onclick="event.stopPropagation(); editCustomer(
                                                <?= $entry['customer_id'] ?>,
                                                '<?= htmlspecialchars($entry['name']) ?>',
                                                '<?= htmlspecialchars($entry['email']) ?>',
                                                '<?= htmlspecialchars($entry['phone']) ?>',
                                                '<?= htmlspecialchars($entry['address']) ?>',
                                                '<?= htmlspecialchars($entry['gender']) ?>',
                                                '<?= htmlspecialchars($entry['race']) ?>'
                                            )"
    ...
    <div class="modal fade" id="addCustomerModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="" method="POST" enctype="multipart/form-data" onsubmit="return validateCustomerPasswords()">
                        <div class="mb-3">
                            <label class="form-label small fw-bold">Full Name <span class="text-danger">*</span></label>
                        ...
                        <button type="submit" name="add_customer" class="btn btn-dark w-100">Create Account</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                <form method="POST" enctype="multipart/form-data" onsubmit="return validateEditCustomerForm()">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" name="edit_customer" class="btn btn-dark">Save Changes</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="col-12">
        <div class="card-custom h-100">
            <div class="card-header-custom bg-white border-bottom">
                ...
                <span class="badge bg-success"><?= count($completed_entries) ?> entries</span>
            </div>
            <div class="p-3" style="max-height: 60vh; overflow-y: auto;" id="completedCustomersList">
                <?php if (empty($completed_entries)): ?>
                    <div class="text-center text-muted py-5">No completed projects yet.</div>
                <?php else: foreach ($completed_entries as $entry):
                    $avatar_src = getAdminCustomerAvatarSrc($entry['profile_image'] ?? '', $entry['name'], $entry['gender'] ?? '', $entry['race'] ?? '');
                    $link_url = ($entry['primary_staff_id']) ? "admin_dashboard.php?view=staff_progress&staff_id=" . $entry['primary_staff_id'] : '#';
                    $onclick = ($entry['primary_staff_id']) ? "window.location.href='$link_url'" : "alert('No staff assigned.')";
                ?>
                    <div class="card mb-3 border shadow-sm customer-card"
                        onclick="<?= $onclick ?>"
                        style="cursor: <?= $entry['primary_staff_id'] ? 'pointer' : 'default' ?>;"
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center gap-3">
                                <img src="<?= $avatar_src ?>" width="50" height="50" class="rounded-circle"
                                    onerror="this.src='https://ui-avatars.com/api/?name=<?= urlencode($entry['name']) ?>&background=28a745&color=fff&size=60&rounded=true&bold=true';">
                                <div class="flex-grow-1">
                                    <h6 class="mb-0 fw-bold"><?= highlightText2($entry['name'], $search) ?></h6>
                                    <div class="small text-muted">ID: #<?= $entry['customer_id'] ?></div>
                                    <div class="small mt-1">
                                    ...
                                    <div class="mt-2">
                                        <span class="badge bg-success">All quotations completed</span>
                                    </div>
                                </div>
                                <div onclick="event.stopPropagation();">
                                    <a href="?view=customers&toggle_customer=<?= $entry['customer_id'] ?>&status=<?= $entry['status'] == 1 ? 0 : 1 ?>"
                                    ...
                                    ?>
                                        <div class="mb-5 pb-3 border-bottom" data-qtn-id="<?= $qtn_id ?>">
                                            ...
                                            <div style="position: relative; margin: 30px 0 20px;">
                                                ...
                                                <?php $progress_percent = ($total_steps > 0) ? round(($continuous_completed / $total_steps) * 100) : 0; ?>
...
<script>
    ...
    document.getElementById('staff_email')?.addEventListener('input', function() {
        validateEmail(this.value, 'staff_email_helper');
    });
    document.getElementById('staff_phone')?.addEventListener('input', function() {
        validatePhone(this.value, 'staff_phone_helper');
    });
    document.addEventListener('DOMContentLoaded', function() {
