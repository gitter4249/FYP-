...
$qtn_query = "SELECT qtn_id FROM quotations WHERE staff_id = $staff_db_id AND status = 'Accepted'";
$qtn_res = mysqli_query($conn, $qtn_query);
...
for ($i = 7; $i >= 0; $i--) {
    ...
    $sql_week = "
        ...
        WHERE status = 'Paid'
        AND staff_id = $staff_db_id
        AND DATE(paid_date) BETWEEN '$week_start' AND '$week_end'
    ";
...
<div class="main-content">
    ...
    <div id="section-progress" class="content-section <?php echo ($active_section == 'progress') ? 'active' : ''; ?>" style="display: <?php echo ($active_section == 'progress') ? 'block' : 'none'; ?>;">
        ...
        $all_customers_sql = "
            SELECT DISTINCT c.customer_id, c.name, c.email, c.profile_image, c.gender, c.race
            FROM customers c
            INNER JOIN quotations q ON c.customer_id = q.customer_id
            WHERE c.status = 1
            AND q.status = 'Accepted'
            AND q.staff_id = $staff_db_id
        ";
        if (!empty($search)) {
            $all_customers_sql .= " AND (c.name LIKE '%$search%' OR c.email LIKE '%$search%')";
        }
        $all_customers_sql .= " ORDER BY c.name ASC";
                                            ...
                                            ?>
                                                <div class="text-center" style="flex: 1;">
                                                    ...
                                                    <?php if ($show_textarea): ?>
                                                        ...
                                                        <small class="text-muted d-block mt-1" style="font-size:0.7rem;">Press Enter to submit note</small>
                                                    <?php endif; ?>
...
<script>
    ...
    window.onload = () => {
        ...
        let active = urlParams.get('active_section');
        if (!active) {
            active = sessionStorage.getItem('activeSection');
        }
        if (!active) {
