...
function generateInvoicePDF($inv_id, $invoice_number, $customer, $items, $total, $issue_date, $qtn_number, $notes_text, $stage_percent = 1.0, $stage_name = '') {
    if (empty($qtn_number)) {
        $qtn_number = 'QT-00000-00';
    }
    ...
    $html .= '<tr>';
    $html .= '<td width="16%">Customer Account</td>';
    $html .= '<td width="12%">Currency</td>';
    $html .= '<td width="18%">From Doc Date</td>';
    $html .= '<td width="20%">From Doc No</td>';
    $html .= '<td width="18%">Invoice No.</td>';
    $html .= '<td width="16%" align="right">Date</td>';
    $html .= '</tr>';
    $html .= '<tr style="font-weight:bold;">';
    $html .= '<td>CUST-' . str_pad($customer['customer_id'] ?? 0, 4, '0', STR_PAD_LEFT) . '</td>';
    $html .= '<td>MYR</td>';
    $html .= '<td>' . $formatted_date . '</td>';
    $html .= '<td>' . htmlspecialchars($qtn_number) . '</td>';
    ...
    $groups = [];
    foreach ($items as $it) {
        $product_name = !empty($it['product_name']) ? $it['product_name'] : 'Unknown Product';
        $desc = $it['description'];
        $area = floatval($it['area']);
        $price = floatval($it['unit_price']);
        $disc = floatval($it['discount']);

        if (!isset($groups[$product_name])) {
            $groups[$product_name] = [
                'subitems' => [],
                'subtotal' => 0,
                'total_area' => 0,
        ...
        $groups[$product_name]['discount'] += $disc;
        $groups[$product_name]['subtotal'] += $area * $price;
        $groups[$product_name]['total_area'] += $area;

        $sub_desc = $desc;
        if (strpos($sub_desc, 'sqft') === false && $area > 0) {
            $sub_desc .= ' (' . number_format($area, 2) . ' sqft)';
        }
        $groups[$product_name]['subitems'][] = [
            'desc' => $sub_desc,
            'area' => $area
        ];
    }
    ...
    foreach ($groups as &$group) {
        $group['total_amount'] = $group['subtotal'] - $group['discount'];
    }
    unset($group);

    $html .= '<table width="100%" cellpadding="3" style="font-size:8pt; font-family:helvetica;">';
    $html .= '<tr>';
    $html .= '<th width="35%" align="left" style="font-size:7pt; color:#666;">Description</th>';
    $html .= '<th width="10%" align="center" style="font-size:7pt; color:#666;">Area (sqft)</th>';
    $html .= '<th width="8%" align="center" style="font-size:7pt; color:#666;">Qty</th>';
    $html .= '<th width="14%" align="right" style="font-size:7pt; color:#666;">Price/Unit<br><span style="font-size:6pt;">(RM)</span></th>';
    $html .= '<th width="12%" align="right" style="font-size:7pt; color:#666;">Discount<br><span style="font-size:6pt;">(RM)</span></th>';
    $percent_display = ($stage_percent < 1.0) ? ' (' . ($stage_percent * 100) . '%)' : '';
    $html .= '<th width="21%" align="right" style="font-size:7pt; color:#666;">Total' . $percent_display . '<br><span style="font-size:6pt;">(RM)</span></th>';
    $html .= '</tr>';

    $scaled_total = 0;
    $counter = 1;
    foreach ($groups as $product => $group) {
        $main_desc = $counter . '. ' . $product;
        $sub_lines = [];
        foreach ($group['subitems'] as $sub) {
            $sub_lines[] = $sub['desc'];
        }
        $full_amount = $group['total_amount'];
        ...
        $html .= '</td>';
        $html .= '<td width="10%" align="center">' . number_format(floatval($group['total_area']), 2) . '</td>';
        $html .= '<td width="8%" align="center">1</td>';
        $html .= '<td width="14%" align="right">' . number_format(floatval($group['unit_price']), 2) . '</td>';
        $html .= '<td width="12%" align="right">' . ($group['discount'] > 0 ? number_format(floatval($group['discount']), 2) : '') . '</td>';
        $html .= '<td width="21%" align="right">' . number_format(floatval($scaled_amount), 2) . '</td>';
        $html .= '</tr>';
        $counter++;
    }

    if ($stage_percent < 1.0) {
        $html .= '<tr><td colspan="5" align="right" style="font-style:italic;">Subtotal (' . ($stage_percent * 100) . '% of quoted amount)</td>';
        $html .= '<td align="right"><strong>' . number_format(floatval($scaled_total), 2) . '</strong></td></tr>';
    }
    $html .= '<tr><td colspan="6" height="40"> </td></tr>';
    $html .= '</table>';
    ...
    $html .= '<b style="font-size:10pt;">' . number_format(floatval($scaled_total), 2) . '</b>';
    $html .= '</td>';
    $html .= '</tr>';
    $html .= '</table>';
    $html .= '<br>';
    ...
    $html .= '3. Goods sold are neither returnable nor refundable.';
    if(!empty($notes_text)) {
