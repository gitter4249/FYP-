...
$sql_user = "SELECT status, name, email, phone, address, profile_image, updated_at, race, gender 
              FROM customers 
              WHERE customer_id = ?";
...
function getDefaultAvatar($gender, $race) {
...
$default_avatar = getDefaultAvatar($user_data['gender'] ?? '', $user_data['race'] ?? '');
...
$sql_inv = "SELECT inv_id, invoice_number, final_amount, issue_date, due_date, status, file_path, qtn_id, stage
            FROM invoices 
            WHERE customer_id = ? 
...
<div class="sidebar">
    ...
    <div class="nav-menu">
        ...
        <ul style="list-style: none; padding: 0; margin: 0;">
            ...
            <li onclick="showPage('Progress', this)"><i class="bi bi-bar-chart-steps"></i> Progress</li>
...
<header class="navbar">
    ...
    <div class="account-area" style="display: flex; align-items: center; gap: 10px;">
        <img src="<?php echo $right_avatar_src; ?>" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover; border: 2px solid #e4e4e7;" onerror="this.onerror=null; this.src='<?php echo $default_avatar; ?>'">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div id="customerPwdMessage" style="display:none;"></div>
                <form id="customerChangePasswordForm">
                    <div class="mb-3">
                        <label class="form-label small fw-bold">Current Password</label>
                        <input type="password" name="current_password" id="customer_current_password" class="form-control" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-bold">New Password</label>
                        <input type="password" name="new_password" id="customer_new_password" class="form-control" required>
                        <div class="password-helper" id="customer_new_password_helper">
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body text-center py-4">
                <img id="modalAvatarPreview" src="<?php echo $avatar_path ?: 'https://ui-avatars.com/api/?name=' . urlencode($name) . '&background=20304a&color=fff&size=150&rounded=true&bold=true'; ?>" alt="Avatar Preview" style="width: 120px; height: 120px; border-radius: 50%; object-fit: cover; border: 3px solid #e4e4e7; margin-bottom: 20px;">
                <div class="d-flex flex-column gap-2">
                    ...
                    <button class="btn btn-outline-danger w-100" onclick="deleteAvatar()">
...
const customerData = {
    ...
    default_avatar: <?php echo json_encode($default_avatar); ?>, 
    updated_at: <?php echo json_encode($user_data['updated_at'] ?? null); ?>
};
...
function filterPaymentHistory() {
    ...
    const cards = document.querySelectorAll('.payment-quotation-card');
    cards.forEach(card => {
        const qtn = (card.getAttribute('data-qtn') || '').toLowerCase();
        const product = (card.getAttribute('data-product') || '').toLowerCase();
        if (filter === '' || qtn.includes(filter) || product.includes(filter)) {
            card.style.display = '';
        } else {
...
function showPage(pageName, element) {
    ...
    try {
        ...
        if(pageName === 'Profile') {
            const defaultAvatar = customerData.default_avatar || `https://ui-avatars.com/api/?name=...`;
            ...
            <div class="address-with-map">
                <input type="text" class="form-control" id="update_address" value="${safeAddress}">
                <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(customerData.address)}" target="_blank" class="map-icon">
            ...
            <div class="card-custom">
                <div class="profile-cover" style="background: linear-gradient(135deg, #000000 0%, #000000 100%);"></div>
                <div style="display: flex; align-items: flex-end; gap: 25px; padding: 0 30px 20px 30px;">
                    ...
                    <div class="profile-info" style="padding-left:0;">
                        <h2 id="profileCardName" style="margin:0;font-size:1.5rem;font-weight:800;">${escapeHtml(customerData.name)}</h2>
                        <p class="text-muted">
                ...
                <div class="p-4">
                    <form style="display:grid;grid-template-columns:1fr 1fr;gap:25px;">
                        <div>
                            ...
                            <input type="text" class="form-control" value="CUST-${String(customerData.id).padStart(4,'0')}" disabled style="background:#f9f9f9;">
                        </div>
                        <div>
                            <label class="small fw-bold">Full Name</label>
                            <input type="text" class="form-control" id="update_name" value="${escapeHtml(customerData.name)}">
                        </div>
                        <div>
                            <label class="small fw-bold">Email</label>
                            <input type="email" class="form-control" value="${escapeHtml(customerData.email)}" disabled style="background:#f9f9f9;">
                        </div>
                        <div>
                            <label class="small fw-bold">Phone</label>
                            <input type="text" class="form-control" id="update_phone" value="${escapeHtml(customerData.phone)}">
                        </div>
                        ...
                        <div style="grid-column:span 2; display:flex; justify-content:space-between; align-items:center;">
                            ...
                            <button type="button" class="btn-dark-custom" onclick="saveProfile(event)">Save Information</button>
                        </div>
                        <div style="grid-column:span 2; margin-top: 10px; font-size: 0.9rem; color: #6c757d; text-align: right;">
                            <i class="bi bi-clock-history me-1"></i> Last updated: ${customerData.updated_at ? new Date(customerData.updated_at).toLocaleString('en-MY', {day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}) : '—'}
                        </div>
                    </form>
                </div>
            </div>
            ...
            for (let q of quotationsWithProgress) {
                ...
                const progressPercent = (continuousCompleted / totalSteps) * 100;
                let timelineHtml = '';
            ...
            if (acceptedQuotations.length === 0) {
                ...
                acceptedQuotations.forEach(q => {
                    ...
                    quotationsHtml += `
                        <div class="card mb-4 payment-quotation-card" data-qtn="${escapeHtml(q.qtn_number)}" data-product="${escapeHtml(productName)}">
                            <div class="card-header bg-light d-flex justify-content-between align-items-center">
                                <div>
                                    <strong>${escapeHtml(q.qtn_number)}</strong> - ${escapeHtml(productName)}
                                </div>
            ...
            const thankYouHtml = `
                <div class="card-custom mb-4" style="background: linear-gradient(135deg, #f0f9ff 0%, #e6f7e6 100%); border-left: 6px solid #22c55e;">
                    <div class="card-body p-4 text-center">
                        ...
                        <a href="https://www.google.com/search?q=yong+sheng+alu+enterprise+&sca_esv=54f2dfe3db26537b&rlz=1C1GCEA_enMY1115MY1115&sxsrf=ANbL-n6l90qhkIuAi8k5AMf8Ax-FoXNGnQ%3A1781093936037&ei=MFYpaun4AbuZjuMP7YOkuAo&ved=0ahUKEwjp0Pn60_yUAxW7jGMGHe0BCacQ4dUDCBA&uact=5&oq=yong+sheng+alu+enterprise+&gs_lp=Egxnd3Mtd2l6LXNlcnAiGnlvbmcgc2hlbmcgYWx1IGVudGVycHJpc2UgMgsQABiABBiiBBiwAzILEAAYgAQYogQYsAMyCBAAGO8FGLADMgsQABiABBiiBBiwAzIIEAAY7wUYsANIzC1Q_yxY_yxwA3gAkAEAmAG4AaABuAGqAQMwLjG4AQPIAQD4AQGYAgOgAhGYAwCIBgGQBgWSBwEzoAeLArIHALgHAMIHBTAuMjIxyAcLgAgB&sclient=gws-wiz-serp#lrd=0x31da6f23026bbdbb:0xb8c7117e1fda0761,3,,,," target="_blank" class="btn btn-success rounded-pill px-4">
...
function deleteAvatar() {
    if (confirm('Are you sure you want to delete your custom avatar? This will revert to the default avatar.')) {
        fetch('delete_avatar.php', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                const defaultSrc = customerData.default_avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(customerData.name)}&background=20304a&color=fff&size=150&bold=true&rounded=true`;
                customerData.photo = '';
                document.getElementById('profileAvatar').src = defaultSrc;
                const modalPreview = document.getElementById('modalAvatarPreview');
                if (modalPreview) modalPreview.src = defaultSrc;
                alert('Avatar deleted successfully');
            } else {
                alert('Failed to delete avatar: ' + data.message);
            }
        })
...
function validateCustomerPassword(inputId, helperId) {
...
function bindCustomerPasswordEvents() {
    const newPwd = document.getElementById('customer_new_password');
    const confirmPwd = document.getElementById('customer_confirm_password');
    if (newPwd) {
        newPwd.removeEventListener('input', customerNewPwdHandler);
        newPwd.addEventListener('input', customerNewPwdHandler);
    }
    if (confirmPwd) {
        confirmPwd.removeEventListener('input', customerConfirmPwdHandler);
        confirmPwd.addEventListener('input', customerConfirmPwdHandler);
    }
}

function customerNewPwdHandler() {
    validateCustomerPassword('customer_new_password', 'customer_new_password_helper');
    ...
    if (confirm.value.length > 0 && confirm.value !== this.value) {
...
function customerConfirmPwdHandler() {
    const pwd = document.getElementById('customer_new_password').value;
    const helper = document.getElementById('customer_confirm_password_helper');
...
document.getElementById('changePasswordModal').addEventListener('show.bs.modal', function () {
    document.getElementById('customer_current_password').value = '';
    document.getElementById('customer_new_password').value = '';
    document.getElementById('customer_confirm_password').value = '';
    document.getElementById('customerPwdMessage').style.display = 'none';
    document.getElementById('customer_new_password').classList.remove('is-invalid');
    document.getElementById('customer_confirm_password').classList.remove('is-invalid');
    document.getElementById('customer_new_password_helper').classList.remove('error');
    document.getElementById('customer_confirm_password_helper').classList.remove('error');
    bindCustomerPasswordEvents();
});

document.getElementById('customerChangePasswordForm').addEventListener('submit', function(e) {
    e.preventDefault();
    const msgDiv = document.getElementById('customerPwdMessage');
    const current = document.getElementById('customer_current_password').value;
    const newp = document.getElementById('customer_new_password').value;
    const conf = document.getElementById('customer_confirm_password').value;
    ...
    if (!isPasswordStrong(newp)) {
        msgDiv.innerHTML = '<div class="alert alert-danger">Password must be at least 8 characters, including uppercase, lowercase, number, and special symbol.</div>';
        msgDiv.style.display = 'block';
        return;
    }
